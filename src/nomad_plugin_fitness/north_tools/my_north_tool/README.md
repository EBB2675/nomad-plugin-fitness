...
Select the appropriate base image for your use case. The nomad-plugin-fitness plugin can be installed on top of your chosen base image during the Docker build process (for this you need to extend the Dockerfile).
...
```bash
docker build -f src/nomad_plugin_fitness/north_tools/my_north_tool/Dockerfile \
    -t ghcr.io/ebb2675/nomad-plugin-fitness:latest .
```

Test the image (for jupyter notebook image):

```bash
docker run -p 8888:8888 ghcr.io/ebb2675/nomad-plugin-fitness:latest
```
